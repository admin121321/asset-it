...
$(document).ready(function() {
    var table = $('.logo_datatable').DataTable({
        ...
        columns: [
            ...
            {data: 'foto_printer', name: 'foto_printer', "render": function (data, type, row, meta) {
                    return '<img src="/images-printer/' + data + '" alt="' + data + '"height="100px" width="100px"/>';
                } },
    ...
    $(document).on('click', '.edit', function(event){
        event.preventDefault(); 
        var serial_number = $(this).attr('serial_number'); alert(serial_number);
        $('#form_result').html('');
 
         
 
        $.ajax({
            url :"/printer-device/edit/"+serial_number+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            processData: false,  
            contentType: false,
    ...
    $('#ok_button').click(function(){
        $.ajax({
            url:"printer-device/destroy/"+serial_number,
            beforeSend:function(){
