            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <p>Lokasi Rak Server</p>
                                <a href="{{ route('rak-server-lokasi.pdf') }}" class="btn btn-primary mb-4">Export PDF</a>
                                <!-- DataTables -->
                                <div align="right">
                                            ...
                                            <div class="modal-dialog">
                                                ...
                                                <form method="post" id="sample_form" enctype="multipart/form-data" class="form-horizontal">
                                                    <div class="modal-header">
                                                        <h5 class="modal-title" id="ModalLabel">Tambah Lokasi Rak Server</h5>
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <select class="form-control" id="rak_id" name="rak_id" aria-label="Floating label select example">
                                                                ...
                                                                @foreach(App\Models\RakServer::all() as $rak)
                                                                    <option value="{{ $rak->id}}" id="rak_id">{{ $rak->brand_rak }} |  {{ $rak->kode_rak }} | {{ $rak->type_rak }}</option>
                                                                @endforeach
                                                            </select>
                                                        </div>
                                                        <div class="form-group">
                                                            <label>Lokasi Rak: </label>
                                                            <input type="text" name="nama_lokasi_rak" id="nama_lokasi_rak" class="form-control" />
                                                        </div>
...
$(document).ready(function() {
    var table = $('.printerdevice_datatable').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('rak-server-lokasi.index') }}",
        columns: [
            {data: 'kode_rak', name: 'kode_rak'},
            {data: 'brand_rak', name: 'brand_rak'},
            {data: 'nama_lokasi_rak', name: 'nama_lokasi_rak'},
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('rak-server-lokasi.store') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('rak-server-lokasi.update') }}";
        }
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            ...
            success:function(data)
            {
                ...
                $('#nama_lokasi_rak').val(data.result.nama_lokasi_rak);
                $('#hidden_id').val(id);
    ...
    $('#ok_button').click(function(){
        $.ajax({
            url:"rak-server-lokasi/destroy/"+id,
            beforeSend:function(){
