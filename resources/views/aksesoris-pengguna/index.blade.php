            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <div class="card">
                            <div class="card-body">
                                <p>Aksesoris Pengguna</p>
                                <a href="{{ route('aksesoris-pengguna.pdf') }}" class="btn btn-primary mb-4">Export PDF</a>
                                <!-- DataTables -->
                                <div align="right">
                                            ...
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <form method="post" id="sample_form" class="form-horizontal">
                                                        ...
                                                        <div class="modal-body">
                                                            <h4 align="center" style="margin:0;">Apakah Anda Yakin Untuk Menghapus ini? jika YA, data Akan kembali ke sisa stok</h4>
                                                        </div>
                                                        <div class="modal-footer">
                                                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
...
$(document).ready(function() {
    ...
    $('#sample_form').on('submit', function(event){
        ...
        if($('#action').val() == 'Add')
        {
            action_url = "{{ route('aksesoris-pengguna.store') }}";
        }
 
        if($('#action').val() == 'Edit')
        {
            action_url = "{{ route('aksesoris-pengguna.update') }}";
        }
    ...
    $(document).on('click', '.edit', function(event){
        ...
        $.ajax({
            url :"/aksesoris-pengguna/edit/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            processData: false,  
            contentType: false,
            ...
            success:function(data)
            {
                ...
                $('#aksesoris_id').val(data.result.aksesoris_id);
    ...
    $('#ok_button').click(function(){
        $.ajax({
            url:"aksesoris-pengguna/destroy/"+id,
            beforeSend:function(){
    ...
    $(document).on('click', '.detailButton', function(event){
        ...
        $.ajax({
            url :"/aksesoris-pengguna/detail/"+id+"/",
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType:"json",
            processData: false,  
            contentType: false,
